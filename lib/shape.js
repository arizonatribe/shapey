...
import {isPlainObj, applyNonTransformProps} from './util'
...
const applyTransforms = curry(
    (spec, input) => pipe(
        pipe(
            when(isPlainObj, filter(is(Function))),
            cond([
                [is(Function), identity],
                [both(isPlainObj, pathSatisfies(regTest(/prop/i), ['shapeyTransforms'])), alwaysEvolve],
                [both(isPlainObj, pathSatisfies(regTest(/whole/i), ['shapeyTransforms'])), mapSpec],
                [isPlainObj, shapeLoosely],
                [T, always]
            ])
        )(spec),
        when(isPlainObj, omit(['shapeyTransforms']))
    )(input)
)
 ...
 * @returns {Object} The input object with the spec transforms applied to it
 */
const makeShaper = curry((spec, input) => pipe(applyTransforms(spec), pruneOutput(spec))(input))
