import {always, apply, curry, curryN, is, map, max, pluck, reduce, unless, values} from 'ramda'
import {safeSpecTransforms} from './util'
...
export default curry((transforms, input) => {
    const spec = map(unless(is(Function), always), safeSpecTransforms(transforms))
    return curryN(
