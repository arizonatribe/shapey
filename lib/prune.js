...
import pipe from 'vanillas/pipe'
import { pick, filter, omit } from 'vanillas/curried'
...
export const keeper = curry((spec, input) =>
  pipe(
    objectify,
    pipe(makePruningSpec, Object.keys, pick)(spec)
  )(input)
)
...
export const keepAndShape = curry((spec, input) =>
  pipe(
    keeper(spec),
    evolveSpec(filter(v => typeof v === 'function', spec))
  )(input)
)
...
export const remover = curry((spec, input) =>
  pipe(
    objectify,
    pipe(makePruningSpec, filter(v => typeof v !== 'function'), Object.keys, omit)(spec)
  )(input)
)
 ...
 */
export const impliedRemove = curry((spec, input) => pick(Object.keys(spec), input))
