 ...
 * In this mode, if you provide any other type of value it won't be used, but will simply signal to shapey
 * that you want that prop kept when the input object is fed through.
 * If the prop _doesn't_ exist, you won't see if on the output
 ...
 * In this mode, if you provide any other type of value it won't be used, but will simply signal to shapey
 * that you want that prop removed when the input object is fed through.
 *
 * @func
 * @sig {k: v} -> {k: v} -> {k: v}
 * @param {Object} spec An object whose values are either transform functions or (preferrably)
...
export const removeAndShape = curry((spec, input) =>
    pipe(
        objectify,
        omit(keys(reject(is(Function), spec))),
        evolveSpec(filter(is(Function), spec))
    )(input)
)
